...
class RecipeController
{
	...
	public function insert($data){
		...
		$recipeTransformer = "{$modelClass}Transformer";
		$recipeTransformer = new $recipeTransformer();

		$query = $QueryTransformer->prepareQuery($data);

		$insert = $model->insert($query);

		return ($insert['code']==200) ? $recipeTransformer->transform($insert, $data) : $insert;
	...
	public function update($data){
		...
		return ($update['code']==200) ? $recipeTransformer->transform($update, $data) : $update;
	...
	public function delete($data){
		...
		return ($delete['code']==200) ? $recipeTransformer->transform($delete, $data) : $delete;
	...
	public function retrieve($data){
		...
		return ($retrieve['code']==200) ? $recipeTransformer->transform($retrieve, $data) : $retrieve;
	...
	public function customQuery($data){
		...
		switch($data['method']):
			...
			case 'getRecipeInById':
					$sql = "SELECT * from recipe WHERE id in (" . $data['recipeIds'] .")";
				break;
